...
struct vprocessor::Memory {
	...
	void Initialize() {
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Initializing emulated memory \"size: 65535 bytes\"\n";
		}
		...
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Done initializing emulated memory\n";
		}
	}
...
struct vprocessor::CPU {
	...
	void Reset(Memory& memory) {
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Initializing emulated processor\n";
		}
		PC = 0x8000;
		SP = 0x7F;
		...
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Done initializing emulated processor\n";
		}
	}

	//* Fetches the 8 bit data currently stored in the memory
	uint8_t FetchByte(uint32_t& cycles, const Memory& memory) {
	...
	uint16_t FetchWord(uint32_t& cycles, const Memory& memory) {
	...
	uint8_t ReadByte(uint32_t& cycles, uint16_t address, const Memory& memory) {
		uint8_t data = memory[address];
		cycles--;
		return data;
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Read value from addresess \"0x" << decToHex(address) << "\"\n";
		}
	}

	//* Reads the 16 bit data currently stored in the memory
	uint16_t ReadWord(uint32_t& cycles, uint16_t address, const Memory& memory) {
		uint8_t Low = ReadByte(cycles, address, memory);
		uint8_t High = ReadByte(cycles, address + 1, memory);
		return Low | (High << 8);
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Read value from addresess \"0x" << decToHex(address) << " - 0x" << decToHex(address++) << "\"\n";
		}
	}

	//* Writes an 8 bit value to memory
	void WriteByte(uint8_t operand, uint32_t& cycles, uint16_t address, Memory& memory) {
		memory[address] = operand;
		cycles--;
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Value written to addresess \"0x" << decToHex(address) << "\"\n";
		}
	}
	...
	void WriteWord(uint16_t operand, uint32_t& cycles, uint16_t address, Memory& memory) {
		...
		cycles -= 2;
		if (isProcessorDebug == true) {
			std::cout << "[DEBUG/CPU_Emulator] Value written to addresess \"0x" << decToHex(address) << " - 0x" << decToHex(address++) << "\"\n";
		}
	}
